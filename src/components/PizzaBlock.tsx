...
const PizzaBlock: React.FC<PizzaBlockProps> = ({
  ...
  return (
    <div className="pizza-block">
      ...
      <h4 className="pizza-block__title">{title ? title : "Pizza"}</h4>
      <div className="pizza-block__selector">
        <ul>
          {types.length === 1 ? (
            <li className="active">{types[0] ? "Traditional" : "Thin"}</li>
          ) : (
            types.map((type, index) => {
              return (
                <li
                  key={index}
                  onClick={() =>
                    pizzaSizeActive(index, setPizzaTypeActiveIndex)
                  }
                  className={pizzaTypeActiveIndex === index ? "active" : ""}
                >
        ...
        <ul>
          {sizes.map((pizzaSize, index) => {
            return (
              <li
                key={index}
                onClick={() => pizzaSizeActive(index, setPizzaSizeActiveIndex)}
                className={pizzaSizeActiveIndex === index ? "active" : ""}
              >
                {pizzaSize}
              </li>
            );
